...
bool DoesReflectionProbeExist()
{
    ...
    return probeExists && !(0.0 /*_BRDFForceFallback*/);
}
...
float3 getDirectSpecular(float roughness, float ndh, float vdn, float ndl, float ldh, float3 f0, float3 halfVector, float3 tangent, float3 bitangent, float anisotropy)
{
        ...
        return lerp(directSpecularNonAniso, directSpecularAniso, saturate(abs((0.0 /*_BRDFAnisotropy*/) * 100))) * 3; // * 100 to prevent blending, blend because otherwise tangents are fucked on lightmapped object
    #else
        return 0;
    #endif
}
float3 getIndirectSpecular(float metallic, float roughness, float3 reflDir, float3 worldPos, float3 lightmap, float3 normal)
{
    float3 spec = float3(0, 0, 0);
        ...
        #if defined(LIGHTMAP_ON)
            float specMultiplier = max(0, lerp(1, pow(length(lightmap), (0.2 /*_SpecLMOcclusionAdjust*/)), (0.0 /*_SpecularLMOcclusion*/)));
            spec *= specMultiplier;
        #endif
    #endif
    return spec;
...
void poiBRDF(inout float4 finalColor, const float4 finalColorBeforeLighting)
{
    ...
    #if defined(PROP_BRDFMETALLICGLOSSMAP) || !defined(OPTIMIZER_ENABLED)
        float4 metallicGlossMap = POI2D_SAMPLER_PAN(_BRDFMetallicGlossMap, _MainTex, poiMesh.uv[(0.0 /*_BRDFMetallicGlossMapUV*/)], float4(0,0,0,0));
    #else
        float4 metallicGlossMap = 1;
    #endif
    #if defined(PROP_BRDFSPECULARMAP) || !defined(OPTIMIZER_ENABLED)
        float4 spcularTintMask = POI2D_SAMPLER_PAN(_BRDFSpecularMap, _MainTex, poiMesh.uv[(0.0 /*_BRDFSpecularMapUV*/)], float4(0,0,0,0));
    #else
        float4 spcularTintMask = 1;
    #endif
    #if defined(PROP_BRDFMETALLICMAP) || !defined(OPTIMIZER_ENABLED)
        float4 metallicTintMask = POI2D_SAMPLER_PAN(_BRDFMetallicMap, _MainTex, poiMesh.uv[(0.0 /*_BRDFMetallicMapUV*/)], float4(0,0,0,0));
    #else
        float4 metallicTintMask = 1;
    #endif
    
    if ((0.0 /*_BRDFInvertGlossiness*/) == 1)
    {
        metallicGlossMap.a = 1 - metallicGlossMap.a;
    }
    float metallic = metallicGlossMap.r * (1.0 /*_BRDFMetallic*/);
    float reflectance = metallicGlossMap.g * (0.495 /*_BRDFReflectance*/);
    float roughness = max(1 - ((0.745 /*_BRDFGlossiness*/) * metallicGlossMap.a), getGeometricSpecularAA(poiMesh.normals[1]));
    finalColor.rgb *= lerp(1, 1 - metallic, (1.0 /*_BRDFReflectionsEnabled*/));
    float3 reflViewDir = getAnisotropicReflectionVector(poiCam.viewDir, poiMesh.binormal, poiMesh.tangent.xyz, poiMesh.normals[1], roughness, (0.0 /*_BRDFAnisotropy*/));
    ...
    #endif
    float3 f0 = 0.16 * reflectance * reflectance * (1.0 - metallic) + lerp(finalColorBeforeLighting.rgb * metallic, 1, (0.0 /*_BRDFMetallicSpecIgnoresBaseColor*/));
    float3 fresnel = lerp(F_Schlick(poiLight.nDotV, f0), f0, metallic); //Kill fresnel on metallics, it looks bad.
    float3 directSpecular = getDirectSpecular(roughness, saturate(poiLight.nDotH), max(poiLight.nDotV, 0.000001), attenuation, saturate(poiLight.lDotH), f0, poiLight.halfDir, poiMesh.tangent.xyz, poiMesh.binormal, (0.0 /*_BRDFAnisotropy*/)) * poiLight.attenuation * attenuation * poiLight.color;
    directSpecular = min(directSpecular, poiLight.color);
    float3 vDirectSpecular = 0;
    #ifdef VERTEXLIGHT_ON
        for (int index = 0; index < 4; index++)
        {
            float3 v0directSpecular = getDirectSpecular(roughness, saturate(poiLight.vDotNH[index]), max(poiLight.nDotV, 0.000001), attenuation, saturate(poiLight.lDotH), f0, poiLight.vHalfDir[index], poiMesh.tangent, poiMesh.binormal, (0.0 /*_BRDFAnisotropy*/)) * poiLight.attenuation * poiLight.vAttenuationDotNL[index] * poiLight.vColor[index];
    ...
    float3 specular = indirectSpecular * (1.0 /*_BRDFReflectionsEnabled*/) * metallicTintMask.a * metallicTintMask.rgb * poiLight.occlusion + (directSpecular + vDirectSpecular) * (1.0 /*_BRDFSpecularEnabled*/) * spcularTintMask.a * spcularTintMask.rgb;
