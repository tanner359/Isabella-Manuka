...
void calculateVertexLightingData(in v2f i)
{
		...
		for (int index = 0; index < 4; index++)
		{
			...
			if ((0.0 /*_LightingAdditiveLimitIntensity*/) == 1)
			{
				float intensity = max(0.001, (0.299 * poiLight.vColor[index].r + 0.587 * poiLight.vColor[index].g + 0.114 * poiLight.vColor[index].b));
				poiLight.vColor[index] = min(poiLight.vColor[index], poiLight.vColor[index] / (intensity / (1.0 /*_LightingAdditiveMaxIntensity*/)));
			}
	...
	poiLight.occlusion = 1;
		...
		poiLight.color = magic + normalLight;
			...
			if ((0.0 /*_LightingAdditiveLimitIntensity*/) == 1)
			{
				...
				poiLight.color = min(poiLight.color, poiLight.color / (additiveLightIntensity / (1.0 /*_LightingAdditiveMaxIntensity*/)));
			}
		#endif
	#endif
	#ifdef FORWARD_BASE_PASS
...
void initializeCamera(v2f i)
{
	...
	poiCam.tangentViewDir.xy /= (poiCam.tangentViewDir.z + (0.42 /*_ParallaxBias*/));
}
...
{
	half3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiMesh.uv[(0.0 /*_BumpMapUV*/)], float4(0,0,0,0)), (2.08 /*_BumpScale*/));
