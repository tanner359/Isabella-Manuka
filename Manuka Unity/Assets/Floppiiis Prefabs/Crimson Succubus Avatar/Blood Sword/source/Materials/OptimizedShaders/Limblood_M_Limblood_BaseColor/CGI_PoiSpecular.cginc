    ...
    float anisoSpec1Alpha, float anisoSpec2Alpha, float4 specularTint, float specularMixAlbedoIntoTint, float4 specularMap, float3 specularLight, float3 lightDirection, float3 halfDir, float nDotL, float jitter, float4 packedTangentMap, in float4 albedo)
{
    ...
    tangentDirectionMap += (0.0 /*_Spec1Offset*/) +jitter;
...
float3 calculateSpecular(in float4 albedo)
{
    ...
    float4 specularMap = POI2D_SAMPLER_PAN(_SpecularMap, _MainTex, poiMesh.uv[(0.0 /*_SpecularMapUV*/)], float4(0,0,0,0));
    half metallic = POI2D_SAMPLER_PAN(_SpecularMetallicMap, _MainTex, poiMesh.uv[(0.0 /*_SpecularMetallicMapUV*/)], float4(0,0,0,0)).r * (1.0 /*_SpecularMetallic*/);
    half specularMask = POI2D_SAMPLER_PAN(_SpecularMask, _MainTex, poiMesh.uv[(0.0 /*_SpecularMaskUV*/)], float4(0,0,0,0)).r;
    float attenuation = saturate(poiLight.nDotL);
    float3 specularLightColor = poiLight.color;
    
    if ((5.0 /*_SpecularMaxBrightness*/))
    {
        specularLightColor = clamp(poiLight.color, 0, (5.0 /*_SpecularMaxBrightness*/));
    }
    #ifdef FORWARD_ADD_PASS
        ...
        if ((4.0 /*_LightingMode*/) == 0 && (0.0 /*_LightingRampType*/) == 1)
        {
            attenuation = poiLight.rampedLightMap;
        }
    #endif
    
    if ((1.0 /*_SpecularType*/) == 1) // Realistic
    {
        if ((0.0 /*_SmoothnessFrom*/) == 1)
        {
            specularMap.a = specularMap.r;
            specularMap.rgb = 1;
        }
        if ((0.0 /*_SpecularInvertSmoothness*/))
        {
            specularMap.a = 1 - specularMap.a;
        }
        finalSpecular += calculateNewSpecular(specularMap.rgb, (0.0 /*_SmoothnessFrom*/), realisticAlbedo, float4(1,0,0,1), metallic, (1.0 /*_SpecularSmoothness*/) * specularMap.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
    }
    
    if ((1.0 /*_SpecularType*/) == 4)
    {
        float jitter = 0;
        float microJitter = POI2D_SAMPLER_PAN(_SpecularAnisoJitterMicro, _MainTex, float2(poiMesh.uv[(0.0 /*_SpecularAnisoJitterMicroUV*/)]), float4(0,0,0,0)).r;
        fixed jitterOffset = (1 - (0.0 /*_SpecularAnisoJitterMirrored*/)) * .5;
        jitter += (POI2D_SAMPLER_PAN(_SpecularAnisoJitterMicro, _MainTex, float2(poiMesh.uv[(0.0 /*_SpecularAnisoJitterMicroUV*/)]), float4(0,0,0,0)).r - jitterOffset) * (0.0 /*_SpecularAnisoJitterMicroMultiplier*/);
        jitter += (POI2D_SAMPLER_PAN(_SpecularAnisoJitterMacro, _MainTex, float2(poiMesh.uv[(0.0 /*_SpecularAnisoJitterMacroUV*/)]), float4(0,0,0,0)).r - jitterOffset) * (0.0 /*_SpecularAnisoJitterMacroMultiplier*/);
        jitter += (0.0 /*_Spec1Offset*/);
        float4 packedTangentMap = POI2D_SAMPLER_PAN(_AnisoTangentMap, _MainTex, poiMesh.uv[(0.0 /*_AnisoTangentMapUV*/)], float4(0,0,0,0));
        finalSpecular += toonAnisoSpecular((0.0 /*_SpecWhatTangent*/), (0.0 /*_AnisoUseTangentMap*/), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, (0.95 /*_SpecularToonStart*/), (1.0 /*_SpecularToonEnd*/), float4(1,0,0,1), albedo, metallic, jitter, (0.0 /*_SpecularAnisoJitterMirrored*/), packedTangentMap);
        finalSpecular *= attenuation;
    }
    #ifdef FORWARD_BASE_PASS
        
        if ((1.0 /*_SpecularType*/) == 2) // Toon
        {
            finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.25,0.3,0,1), metallic, (0.0 /*_SmoothnessFrom*/), specularMap, specularLightColor, poiMesh.normals[(1.0 /*_SpecularNormal*/)], poiLight.halfDir, poiLight.attenuation);
            finalSpecular *= float4(1,0,0,1);
        }
        
        if ((1.0 /*_SpecularType*/) == 3) // anisotropic
        {
            float jitter = 0;
            float microJitter = POI2D_SAMPLER_PAN(_SpecularAnisoJitterMicro, _MainTex, float2(poiMesh.uv[(0.0 /*_SpecularAnisoJitterMicroUV*/)]), float4(0,0,0,0)).r;
            fixed jitterOffset = (1 - (0.0 /*_SpecularAnisoJitterMirrored*/)) * .5;
            ...
            jitter += (0.0 /*_Spec1Offset*/);
            float4 packedTangentMap = POI2D_SAMPLER_PAN(_AnisoTangentMap, _MainTex, poiMesh.uv[(0.0 /*_AnisoTangentMapUV*/)], float4(0,0,0,0));
            finalSpecular += AnisotropicSpecular((0.0 /*_SpecWhatTangent*/), (0.0 /*_AnisoUseTangentMap*/), (1.0 /*_SpecularSmoothness*/), (0.0 /*_Spec2Smoothness*/), (1.0 /*_AnisoSpec1Alpha*/), (1.0 /*_AnisoSpec2Alpha*/), float4(1,0,0,1), metallic, specularMap, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular *= float4(1,0,0,1);
            finalSpecular *= attenuation;
        }
    #endif
        ...
        for (int index = 0; index < 4; index++)
        {
            ...
            if ((1.0 /*_SpecularType*/) == 1) // Realistic
            {
                finalSpecular += calculateNewSpecular(specularMap.rgb, (0.0 /*_SmoothnessFrom*/), realisticAlbedo, float4(1,0,0,1), metallic, (1.0 /*_SpecularSmoothness*/) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
            }
        }
    #endif
    finalSpecular *= float4(1,0,0,1).a;
