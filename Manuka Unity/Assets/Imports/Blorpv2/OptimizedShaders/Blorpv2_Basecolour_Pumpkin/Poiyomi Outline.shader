Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/790f31b29fc9acd4e8329d78558aa89d"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			v2f vert(appdata v)
			{
				...
				float lineWidth = (0.0008 /*_LineWidth*/);
				#ifdef POI_AUDIOLINK
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				{
					currentFrame = fmod((-1.0 /*_FlipbookCurrentFrame*/), totalFrames);
				}
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, (1.0 /*_RimPower*/));
				if ((0.0 /*_RimShadowWidth*/) && (0.0 /*_RimShadowToggle*/))
				{
					viewDotNormal += lerp(0, (1 - poiLight.nDotLNormalized) * 3, (0.0 /*_RimShadowWidth*/));
				}
				float rimStrength = (0.14 /*_RimStrength*/);
				float rimWidth = lerp( - .05, 1, (0.8 /*_RimWidth*/));
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[(0.0 /*_RimTexUV*/)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, _RimLightColor.rgb, (0.0 /*_RimLightColorThemeIndex*/)), _RimLightColor.a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, _RimLightColor.rgb, (0.0 /*_RimLightColorThemeIndex*/)), _RimLightColor.a);
				#endif
				...
				float rim = 1 - smoothstep(min((0.0 /*_RimSharpness*/), rimWidth), rimWidth, viewDotNormal);
				rim *= _RimLightColor.a * rimColor.a * rimMask;
				if ((0.0 /*_RimShadowToggle*/))
				{
					switch((0.0 /*_RimShadowMaskRampType*/))
					{
						case 0: rim = lerp(rim, rim * poiLight.rampedLightMap, (1.0 /*_RimShadowMaskStrength*/)); break;
						case 1: rim = lerp(rim, rim * smoothstep(float4(0,0,0,1).x, float4(0,0,0,1).y, poiLight.nDotLNormalized), (1.0 /*_RimShadowMaskStrength*/)); break;
					}
				}
				float3 finalRimColor = rimColor.rgb * lerp(1, poiFragData.baseColor, (1.0 /*_RimBaseColorMix*/));
				finalRimColor *= (1.0 /*_RimBrightness*/);
				switch((0.0 /*_RimBlendMode*/))
				{
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if ((3.0 /*_LightingColorMode*/) == 3)
				{
					...
					poiLight.indirectColor = max(poiLight.indirectColor, (0.1 /*_LightingMinLightBrightness*/));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / (0.1 /*_LightingMinLightBrightness*/))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / (0.1 /*_LightingMinLightBrightness*/))));
				}
				...
				poiLight.attenuation = attenuation;
				#endif
				poiLight.directColor = (1.0 /*_LightingAdditiveLimited*/) ? min((0.8 /*_LightingAdditiveLimit*/), _LightColor0.rgb) : _LightColor0.rgb;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[(0.0 /*_OutlineTextureUV*/)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.1094617,0.01444384,0.006512089,1).rgb, (0.0 /*_LineColorThemeIndex*/)), float4(0.1094617,0.01444384,0.006512089,1).a);
				poiFragData.baseColor = lerp(col, col * poiFragData.baseColor, (1.0 /*_OutlineTintMix*/));
				if ((1.0 /*_OutlineExpansionMode*/) == 2)
				{
